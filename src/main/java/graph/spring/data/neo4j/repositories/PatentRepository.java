...
public interface PatentRepository extends GraphRepository<Patent> {
    @Query("MATCH (p:Patent) USING INDEX p:Patent(PatentId) WHERE p.PatentId = {patentId} RETURN p")
    Patent findByPatentId(@Param("patentId") String patentId);

    @Query("MATCH (a:Author)-[:AUTHORED_BY]->(p:Publication)<-[:LITERATURE_PATENT_CITATION]-(pa:Patent) USING INDEX pa:Patent(PatentId) WHERE pa.PatentId = {patentId} RETURN a")
